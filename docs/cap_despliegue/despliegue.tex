...
\begin{itemize}
  \item \textbf{\foreign{english}{Logs}}: Se trata de eventos de la aplicación que se registran durante su funcionamiento. Pueden almacenarse como simples cadenas de texto o estructuras de datos más complejas. En el segundo caso, se trata de registros enriquecidos con propiedades que les dotan de más contexto. Es el mecanismo de telemetría que ofrece más detalle del funcionamiento de un servicio concreto. También es el más usado.
...
Para explotar estos datos, necesitaremos poder hacer consultas sobre ellos. Pongamos por ejemplo que hemos detectado que ha aumentado considerablemente la métrica de la duración media de las peticiones. A partir de la fecha y hora de este suceso, deberíamos poder recuperar la información necesaria para responder a la pregunta de qué ha pasado. Ya sean \foreign{english}{logs}, trazas u otras métricas relacionadas.

Una \textbf{plataforma de observabilidad} es un conjunto de servicios que nos permite centralizar la gestión de los datos.

Para poder capturar y explotar esta información debimos construir nuestra propia \textbf{plataforma de observabilidad}. Esta nos permitió hacer consultas sobre el estado del sistema y crear vistas que nos permitan conocerlo. También pudimos correlacionar los datos de las metricas, trazas y logs ara hacer consultas

\begin{wrapfigure}{r}{0.16\linewidth}
  \centering
  \vspace{-10pt}
  \includegraphics[scale=0.40]{cap_despliegue/images/opentelemetry-logo}
  \vspace{-10pt}
\end{wrapfigure}

Para poder capturar todas estas señales, empleamos el estándar \textbf{OpenTelemetry}\footnote{Página oficial: \url{https://opentelemetry.io/}}. Se trata de un proyecto desarrollado por la Cloud Native Computing Foundation (CNCF). Tiene el objetivo de definir un mecanismo estándar para recopilar y transmitir datos de telemetría. Para ello, ofrece un conjunto de librerias que nos permite instrumentar nuestras aplicaciones. Podremos enviar estos datos a cualquier plataforma que ofrezca extensiones compatibles.

Para poder capturar y explotar estas señales, necesitaremos construir nuestra plataforma de observabilidad.
