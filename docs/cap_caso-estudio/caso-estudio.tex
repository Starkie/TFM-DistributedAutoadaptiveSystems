...
El monitor de la solución expone endpoints para recabar las mediciones de las sondas. En este caso, las mediciones de temperatura del aire acondicionado: \texttt{POST Measurement/Temperature}. Este endpoint recibe la temperatura y ejecuta las validaciones del monitor descritas en el apartado anterior. Si el valor es válido, lo manda al servicio de monitorización.

También ofrece endpoints para actualizar la configuración del servicio de aire acondicionado. Así podrá registra su configuración inicial o los modos de operación cuando cambien. Son un

\subsection{Reglas}

En cuanto al servicio de reglas, este contiene las cuatro reglas definidas en el apartado \ref{sec:caso-estudio-diseño-reglas}. Se han implementado siguiendo la estructura descrita en el apartado \ref{sec:implementacion-modulo-reglas}: Todas ellas heredan de las clase abstracta \texttt{AdaptionRuleBase}. Deben implementar los métodos para evaluar la condición y ejecutar su acción; y deben declarar las propiedades de adaptación de las que dependen.

Mediante atributos las reglas describen las propiedades de las que dependen, aquellas que aparecen en su condición. Tomemos por ejemplo el fragmento \ref{ls:adaption-rule-dependencies}. Vemos que la declaración de \texttt{DisableAirConditionerWhenCoolingAndTargetTemperatureAchievedAdaptionRule} depende de tres propiedades o configuraciones: \texttt{airconditioner-mode}, \texttt{temperature} y \texttt{hot-temperature-threshold}.

Nos centraremos en la regla \texttt{DisableAirConditionerWhenCoolingAndTargetTemperatureAchievedAdaptionRule}, que ya habíamos mencionado en apartados anteriores. Ya habíamos mostrado su método \texttt{Execute} en el fragmento \ref{ls:change-request-builder}.

Solo nos queda exponer la implementación de referencia del método \texttt{EvaluateCondition}. En el fragmento \ref{ls:adaption-rule-evaluate-condition} ofrecemos la implementación de la condición \emph{airconditioner-mode} == \emph{Cooling} \textbf{AND} \emph{temperature} <= \emph{target-temperature}. Observamos que en las líneas 3-5, 12-15 y 17-20 se obtienen las propiedades de adaptación o claves de configuración desde el servicio de análisis. En base a ellas, en las líneas 22-23 se evalúa la condición descrita.
...
Finalmente, tenemos el servicio de efectores. Este servicio captura las solicitudes de ejecución emitidas por el módulo de ejecución y determina cómo deberán llevarse a cabo. Para el caso de estudio, por restricciones de tiempo, nos limitamos a implementar las adaptaciones que implicaban cambios en los parámetros del sistema.

Este tipo de adaptaciones son muy dependientes del sistema manejado. Por ejemplo, para el servicio de aire acondicionado, cambiar el modo de operación supone invocar a los endpoints descritos anteriormente. Por ejemplo: (\texttt{POST /airconditioner/turn-off}).
