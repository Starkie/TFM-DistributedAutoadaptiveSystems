...
Otra estrategia posible es \emph{publish-suscribe}: sirve para implementar comunicación \emph{multicast}. Se basa en el uso de \textbf{temas} o \textbf{\emph{topics}}: categorías de mensajes que pueden resultar de interés. Un servicio (el productor) envía un mensaje al \emph{broker}, indicando que pertenece a un tema determinado. El \emph{broker} recibe el mensaje y se encarga de reenviarlo a todos los servicios subscritos a este tema en concreto. \cite{rabbitmqPublishSubscribeDocumentation} En la figura \ref{fig:publish-subscribe} tenemos un ejemplo de esta estrategia.
...
El siguiente mecanismo de comunicación a implementar son las notificaciones asíncronas. Recordemos que esta comunicación es desde un servicio ''a la capa superior''. Esta comunicación no debe estar acoplada a ningún servicio concreto.
