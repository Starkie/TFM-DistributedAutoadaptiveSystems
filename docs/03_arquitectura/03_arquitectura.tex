...
Como comentamos antes, vamos a inspirarnos en los mecanismos de comunicación descritos por C2: las peticiones y notificaciones. Pero estos no cubren todas nuestras necesidades. Hay dos casos que no están contemplados: la comunicación del módulo de análisis con el planificador, y la del planificador con el ejecutor. Estos módulos se encuentran en la misma capa. Y, como dependen del conocimiento, no podemos moverlos a una capa superior para utilizar notificaciones. Por ello, requerimos de un tercer patrón de comunicación: \textbf{colas de trabajo}. \cite{royChapterMessagePatterns2017}
...
Las notificaciones no nos sirven, ya que la comunicación es entre dos módulos específicos. Aunque nos interesa el desacoplamiento entre módulos que ofrecen. Las peticiones tampoco casan del todo, aunque la comunicación es dirigida ya que no queremos acoplar los módulos. Deberían mantener su independencia en el mayor grado posible.
...
\begin{itemize}
  ...
  \item \textbf{Peticiones asíncronas}: Comunicaciones asíncronas dirigidas a un servicio determinado. Solo están permitidas entre elementos del mismo nivel. Se trata de peticiones de trabajo de los cuales no requerimos una respuesta. Se envían y ya se procesarán cuando corresponda.
